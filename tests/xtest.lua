...
local function stringify_recursive(value, indentation, table_set)
    local t = type(value)
    if t == "table" and not table_set[value] then
        table_set[value] = true
        local writer = {"{"}
        local next_indentation = indentation.."    "
        for k, v in pairs(value) do
            writer[#writer+1] = "\n"
            writer[#writer+1] = next_indentation
            writer[#writer+1] = stringify_recursive(k, next_indentation, table_set)
            writer[#writer+1] = " = "
            writer[#writer+1] = stringify_recursive(v, next_indentation, table_set)
            writer[#writer+1] = ","
        end
        writer[#writer+1] = "\n"
        writer[#writer+1] = indentation
        writer[#writer+1] = "}"
...
local function stringify(value)
  return stringify_recursive(value, "", {})
end

xtest.stringify = stringify

function xtest.debug(value)
